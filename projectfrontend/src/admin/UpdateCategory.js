...
const UpdateCategory = ({ match }) => {
  ...
  const preLoad = (categoryId) => {
    getCategory(categoryId, user._id).then((data) => {
      console.log(data);
      if (data.error) {
  ...
  const onsubmit = (event) => {
    ...
    setError("");
    setSuccess(false);
    updateCategory(match.param.categoryId, user._id, token, { name }).then(
      (data) => {
        if (data.error) {
          setError(true);
        } else {
    ...
    <div>
      ...
      >
        <div className="row bg-white rounded">
          {goBack()}
          <div className="col-md-8 offset-md-2">{UpdateCategory()}</div>
        </div>
      </Base>
    </div>
  );
